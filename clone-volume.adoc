...
summary: 在NetApp Workload Factory 中克隆一个卷，以制作原始卷的读/写卷进行测试。 
---
...
[role="lead"]
在NetApp Workload Factory 中克隆一个卷，以制作原始卷的读/写卷进行测试。
...
. 从 *FSx for ONTAP* 中，选择包含要克隆的卷的 FSx for ONTAP文件系统的操作菜单，然后选择 *管理*。
. 从文件系统的概述选项卡中，选择*卷*选项卡。
. 在“卷”选项卡中，选择要克隆的卷的操作菜单。
. 选择*数据保护操作*，然后选择*克隆卷*。
. 在克隆卷对话框中、输入卷克隆的名称。
. 选择 * 克隆 * 。
