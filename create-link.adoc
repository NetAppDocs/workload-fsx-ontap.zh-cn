...
.开始之前
* 您应考虑使用哪种链接创建选项。
* 您需要在 Workload Factory 中至少有一个 FSx for ONTAP文件系统。要发现或创建 FSx for ONTAP文件系统，您必须拥有一个具有 FSx for ONTAP实例权限的 AWS 账户，并且link:https://docs.netapp.com/us-en/workload-setup-admin/add-credentials.html#overview["在 Workload Factory 中添加凭证"^]具有存储管理的_只读_或_读/写_权限。
