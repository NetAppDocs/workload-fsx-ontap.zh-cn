...
---
= 在NetApp Workload Factory 中暂停和恢复复制关系
:allow-uri-read: 
:icons: font
:imagesdir: ./media/
...
当您恢复复制时，对目标卷的所有更改都将被撤消，并且NetApp Workload Factory 将重新启用复制。目标卷从读/写转换为只读，并再次按照计划的复制间隔从源卷接收更新。当您恢复复制关系时，目标卷将恢复到最新的初始复制快照，此时，卷复制过程重新开始。
