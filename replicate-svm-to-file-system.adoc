...
[role="lead"]
在NetApp Workload Factory 中将存储虚拟机复制到另一个 FSx for ONTAP文件系统可在数据丢失时提供数据访问的保护层。此操作将一个存储虚拟机中的所有卷复制到另一个 FSx for ONTAP文件系统。

.开始之前
要将存储虚拟机复制到另一个 FSx for ONTAP 文件系统，您必须关联一个链接。link:https://docs.netapp.com/us-en/workload-setup-admin/create-link.html["了解如何关联现有链接或创建并关联新链接"^]。链接关联后，返回此操作。
...
. 从 *FSx for ONTAP* 中，选择要复制存储虚拟机的文件系统的操作菜单，然后选择 *管理*。
. 在“存储虚拟机”下的文件系统概览中，选择“管理”。
. 在“管理存储虚拟机”屏幕上，选择要为其复制 SVM 的存储虚拟机的操作菜单，然后选择“高级操作”>“复制存储虚拟机”。
...
+
使用 Workload Factory FSx for ONTAP用户界面创建卷时，_Auto_ 是默认分层策略。有关卷分层策略的更多信息，请参阅link:https://docs.aws.amazon.com/fsx/latest/ONTAPGuide/volume-storage-capacity.html#data-tiering-policy["卷存储容量"^]在 AWS FSx for NetApp ONTAP文档中。
