...
summary: 在 NetApp Workload Factory 中发现和复制本地 ONTAP 数据，以便将其用作云中的备份。 
---
= 在NetApp Workload Factory 中使用本地ONTAP集群数据
:allow-uri-read: 
:icons: font
:imagesdir: ./media/
...
[role="lead"]
在 NetApp Workload Factory 中发现和复制本地 ONTAP 数据，以便将其用作云中的备份。
...
在NetApp Workload Factory 中发现本地ONTAP集群，以便您可以将数据复制到Amazon FSx for NetApp ONTAP文件系统。

.开始之前
开始之前、请确保您已具备以下条件：

* 用于复制的FSx for ONTAP文件系统。
* 与发现的本地集群关联的已连接链接。如果您没有链接，则需要 link:https://docs.netapp.com/us-en/workload-setup-admin/create-link.html["创建一个"^]。
* 具有所需权限的ONTAP用户凭据。
* 内部ONTAP 9.8及更高版本。
* 连接、如下图所示。
+
...
+
.. *链接*：选择一个链接。该链接将与内部部署集群相关联，以在集群和工作负载工厂之间建立连接。
+
如果尚未创建链接、请按照说明进行操作、然后返回到此操作并选择此链接。

.. *集群IP地址*：提供要复制的内部ONTAP集群的IP地址。
.. *ONTAP credentials*：提供以下信息：
+
*** GovCloud 账号：输入带有 ONTAP 凭据的 ARN。
*** 所有其他帐户：输入本地 ONTAP 集群的 ONTAP 凭据。确保用户具有所需的权限。
...
* 用于复制关系的本地 ONTAP 集群和 FSx for ONTAP 文件系统都必须具有关联的链接。link:https://docs.netapp.com/us-en/workload-setup-admin/create-link.html["了解如何关联现有链接或创建并关联新链接"^]。链接关联后，返回此操作。


.步骤
. 使用其中一个登录link:https://docs.netapp.com/us-en/workload-setup-admin/console-experiences.html["控制台体验"^]。
. 选择菜单 image:workloads-menu-icon.png["汉堡菜单图标用于导航至存储、EDA、AI、数据库、VMware 和管理等工作负载。"] 然后选择“存储”。
. 从存储菜单中，选择 *On-premises ONTAP*。
...
+
*** *自动*：使用 Workload Factory FSx for ONTAP用户界面创建卷时的默认分层策略。将包括用户数据和快照在内的所有冷数据分层到容量池存储层，保存特定时间段。
...
== 从NetApp Workload Factory 中删除本地ONTAP集群

需要时从NetApp Workload Factory 中删除本地ONTAP集群。
...
. 从存储菜单中，选择 *On-premises ONTAP*。
. 选择要删除的内部ONTAP集群。
. 选择操作菜单并选择*从工作负载工厂中删除*。


.结果
本地ONTAP集群已从NetApp Workload Factory 中删除。
