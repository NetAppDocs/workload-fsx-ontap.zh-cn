...
summary: 使用NetApp Workload Factory，您可以创建 FSx for ONTAP文件系统来添加和管理卷和其他数据服务。 
---
= 在NetApp Workload Factory 中创建 FSx for ONTAP文件系统
:allow-uri-read: 
:icons: font
:imagesdir: ./media/
...
[role="lead"]
使用NetApp Workload Factory，您可以创建第一代和第二代 FSx for ONTAP文件系统来添加和管理卷和其他数据服务。
...
* IAM 角色的 ARN，该角色为 Workload Factory 提供创建 FSx for ONTAP文件系统所需的权限。link:https://docs.netapp.com/us-en/workload-setup-admin/add-credentials.html["了解如何为AWS帐户授予权限"^] 。
